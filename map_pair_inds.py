...
pairs = []
with open("mm10-TSS-TTS-pubWTmESC-mapped-filt.pairs") as f:
    for line in f:
        current = line.strip('\n').split('\t')
...
tss = {}
with open("mm10-TSS-TTS.bed") as f:
    for ind,line in enumerate(f):
        if ind > 0:
            current = line.strip('\n').split('\t')
            if current[0] not in tss.keys():
                tss[current[0]] = [int(current[1])]
            else:
                tss[current[0]].append(int(current[1]))
print('Sites loaded...')
